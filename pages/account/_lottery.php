...
<b>Правила:</b> Всего имеется <?=$num_bil; ?> билетов. После того, как все билеты будут проданы состоится розыгрыш счастливых билетов. Система случайным образом выберет 3 номера счастливых билетов и зачислит им призы. <BR />
1 место - 50% от общего банка [<?=($amount_lottery * $num_bil) * 0.5; ?><font color="blue"><b>C</b></font>]. <BR />
2 место - 25% от общего банка [<?=($amount_lottery * $num_bil) * 0.25; ?><font color="blue"><b>C</b></font>]. <BR />
3 место - 20% от общего банка [<?=($amount_lottery * $num_bil) * 0.2; ?><font color="blue"><b>C</b></font>]. <BR />
Остальные 5% составляют комиссию системы.
<BR />
<u>Стоимость билета = <?=$amount_lottery; ?><font color="blue"><b>C</b></font></u>.
<BR />
...
<?PHP
	...
	if(isset($_POST["set_lottery"], $_POST["hash"]) AND $_SESSION["lot_hash"] == $_POST["hash"]){
		...
		}else echo "<center><b><font color='red'>Покупка более 1 билета запрещена</font></b></center><BR />";
...
<table cellpadding='3' cellspacing='0' border='0' bordercolor='#336633' align='center' width="99%">
  <tr>
    <td colspan="5" align="center"><h1>Пользователи купившие билеты</h1></td>
    </tr>
  <tr>

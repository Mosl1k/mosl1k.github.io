...
$cena = 100; //Стоимость отправки сообщения пользователю
$cena_ref = 250;// Стоимость отправки сообщения рефералам!
?>
<div id="b3">
	<div class="block">
		<h2>Внутреняя почта</h2>
...
$tema1 = $db->RealEscape($_POST['tema1']);
$text1 = $db->RealEscape($_POST['message1']);
$user = $db->RealEscape($_POST['to_user1']);
...
if(!empty($tema1)) {
    if(!empty($text1)) {
                        ...
                        $db->Query("UPDATE db_users_b SET money_p = money_p - '$cena' WHERE id = '$usid'");
                    //    echo $tema1;
...
$text = $db->RealEscape($_POST['message']);
$tema = $db->RealEscape($_POST['tema']);
$date = time();
$status = 0;
    if(!empty($tema)) {
        if(!empty($text)) {
                ...
                if($ref_data['all_users'] !=0) {
                    ...
                    //$db->Query("SELECT * FROM db_users_a WHERE referer_id = '$usid'");
                        ...
                        $cl = mysql_select_db($config->BaseDB, $con);
                            ...
                            $db->Query("UPDATE db_users_b SET money_p = money_p - '$cena_ref' WHERE id = '$usid'");
                            //echo $cena_ref;
                            }
...
<br>
Стоимость отправки 1 сообщение 1 рефералу - 250 Cер.
<br><br>
<?
...
$to_user = $db->RealEscape($_POST['to_user']);
$theme = $db->RealEscape($_POST['tema']);
$text = $db->RealEscape($_POST['message']);
...
<br>
Стоимость отправки сообщения - 100 Cер.
<br><br>
<form method="post" action="">
<label>Пользователь(логин)</label><br>

			...
			if(is_numeric($_POST['ocharsum']) && $_POST['ocharsum']>1) {
				...
				if($db->NumRows() > 0) {
					# Начисляем баланс
						...
						$data=$db->FetchArray();
						if(intval($data["insert_sum"])<=0.01){$sum=$sum+$sum*0.05;} // если первое - +20% юзеру
						...
						if(!empty($data["referer"])) {
							...
							$db->Query("UPDATE db_users_b SET a_t = a_t + 1 WHERE id = '$rid'"); // + дерево
				...
				if($db->FetchRow() == 1){
					...
					$db->Query("UPDATE db_competition_users SET points = points + '{$rub}' WHERE user_id = '{$rid}'");
							...
							//$db->Query("UPDATE db_users_b SET from_referals = from_referals + {$rsum} WHERE id = '$rid'"); // + в лог рефереру
							$refstr=", реферер $rname получил $rsum серебра и 1 лайм";
						}
						$wmset = new wmset();
						$marray = $wmset->GetSet($rub);
...
if(preg_match('#[a-g]{1}+_t#', $_POST["set_tree"])) {
	...
	{
		echo "<center><b>Дерево не добавлено</b></center><BR />";
	}
}
...
if(isset($_POST["auto_bal"]) AND !empty($_POST["a_sum"]))
{
	...
	$rub=intval($_POST["a_sum"]);$sum = $rub*100;
	$data=$db->FetchArray();
	if(intval($data["insert_sum"])<=0.01){$sum=$sum+$sum*0.05;} // если первое - +20% юзеру
	...
	if(!empty($data["referer"]))
	{
		$rid=$data["referer_id"];$rname=$data["referer"];$rsum=$sum*0.1;
		$db->Query("UPDATE db_users_b SET money_b = money_b + {$rsum} WHERE id = '$rid'"); // + 20% рефереру
		$db->Query("UPDATE db_users_b SET a_t = a_t + 1 WHERE id = '$rid'"); // + дерево
		$db->Query("UPDATE db_users_b SET to_referer = to_referer + {$rsum} WHERE id = '$eid'"); // + в лог юзеру
		$db->Query("UPDATE db_users_b SET from_referals = from_referals + {$rsum} WHERE id = '$rid'"); // + в лог рефереру
...
<table width="100%" border="0">
  ...
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Кораблей на балансе (Шнява):</td>
    <td width="200" align="center"><?=$data["a_b"]; ?></td>
  </tr>
  <tr>
    <td style="padding-left:10px;">Кораблей на балансе (Флейт):</td>
    <td width="200" align="center"><?=$data["b_b"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Кораблей на балансе (Тартана):</td>
    <td width="200" align="center"><?=$data["c_b"]; ?></td>
  </tr>
  <tr>
    <td style="padding-left:10px;">Кораблей на балансе (Пинас):</td>
    <td width="200" align="center"><?=$data["d_b"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Кораблей на балансе (Люгер):</td>
    <td width="200" align="center"><?=$data["f_b"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Кораблей на балансе (Клипер):</td>
    <td width="200" align="center"><?=$data["e_b"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Кораблей на балансе (Каравелла):</td>
    <td width="200" align="center"><?=$data["g_b"]; ?></td>
  </tr>
  
  
  <tr>
    <td style="padding-left:10px;">Кораблей (Шнява):</td>
    <td width="200" align="center">
  ...
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Кораблей (Флейт):</td>
    <td width="200" align="center">
  ...
  <tr>
    <td style="padding-left:10px;">Кораблей (Тартана):</td>
    <td width="200" align="center">
  ...
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Кораблей (Пинас):</td>
    <td width="200" align="center">
  ...
  <tr>
    <td style="padding-left:10px;">Кораблей (Люгер):</td>
    <td width="200" align="center">
  ...
  <tr>
    <td style="padding-left:10px;">Кораблей (Клипер):</td>
    <td width="200" align="center">
  ...
  <tr>
    <td style="padding-left:10px;">Кораблей (Каравелла):</td>
    <td width="200" align="center">
  ...
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Собрано за все время (Шнява):</td>
    <td width="200" align="center"><?=$data["all_time_a"]; ?></td>
  </tr>
  <tr>
    <td style="padding-left:10px;">Собрано за все время (Флейт):</td>
    <td width="200" align="center"><?=$data["all_time_b"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Собрано за все время (Тартана):</td>
    <td width="200" align="center"><?=$data["all_time_c"]; ?></td>
  </tr>
  <tr>
    <td style="padding-left:10px;">Собрано за все время (Пинас):</td>
    <td width="200" align="center"><?=$data["all_time_d"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Собрано за все время (Люгер):</td>
    <td width="200" align="center"><?=$data["all_time_f"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Собрано за все время (Клипер):</td>
    <td width="200" align="center"><?=$data["all_time_e"]; ?></td>
  </tr>
  <tr bgcolor="#efefef">
    <td style="padding-left:10px;">Собрано за все время (Каравелла):</td>
...
<table width="100%" border="0">
  ...
  <tr bgcolor="#55CCCC">
    <td align="center" colspan="3">INSERT_SUM:
    <input type="text" name="i_sum" value="" size="7"/></td>
    <td align="center" colspan=""><input type="submit" name="i_bal" value="Выполнить" /></td>
  </tr>
</table>
</form>
</div>
...
<table cellpadding='3' cellspacing='0' border='0' bordercolor='#336633' align='center' width="99%">
  <tr bgcolor="#efefef">
	...
	<td align="center" width="75" class="m-tb"><a href="/?menu=admin384&sel=users&sort=3" class="stn-sort">Кораблей</a></td>
